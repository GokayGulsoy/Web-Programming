/*
  ...
  all the necessary React 
  components 
  NOTE: I have implemented
  menuList without reading file,
  as a array of objects, but you
  can still remove or add some elements
  inside the array and it will re-render correctly
  aligning 3 items per column
*/
import './index.css';
...
/* Header component*/
function Header({ notPurpleContactInfo, notPurpleLocationInfo, personNameSet, aboutUs, aboutUsHandle, aboutUsHandle2, setnotPurpleAboutUs, setPersonName, foodCount1, foodCount2 }) {
   return (
    <header> 
      <div className="header_h1">
         <h1 id="header">Gülbahçe Burger</h1>
...
/*AboutUs component*/
function AboutUs({ notPurpleContactInfo, notPurpleLocationInfo, personNameSet, aboutUs, aboutUsHandle, aboutUsHandle2, setnotPurpleAboutUs, foodCount1, foodCount2 }) {
...
function MenuItem({ imageClass,itemName, itemPrice, picturePath, specialStyle} ) {
   return (
      <div> 
       <div className={imageClass}><img src={picturePath} width="300" height="200" alt={itemName} />
       <p className="food_paragraph">{itemName}<span className="food_span" style={specialStyle}>{itemPrice}</span></p></div>
      </div>          
  );
}
